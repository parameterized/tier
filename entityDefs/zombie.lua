...
for _, sc in ipairs{'server', 'client'} do
    zombie[sc].newDefaults = function()
        local t = {
            id = lume.uuid(),
            x = 0, y = 0,
            xv = 0, yv = 0,
    ...
    zombie[sc].destroy = function(self)
    ...
    zombie[sc].type = 'zombie'
    zombie[sc].static = false
    zombie[sc].enemy = true
end



function zombie.server:serialize()
    local t = {}
    for _, v in ipairs{
        'id', 'type',
        'x', 'y', 'xv', 'yv',
...
function zombie.server:update(dt)
...
function zombie.server:damage(d, clientId)
    self.hp = self.hp - d
...
function zombie.client:setState(state)
...
function zombie.client:lerpState(a, b, t)
    local state = {}
...
function zombie.client:update(dt)
...
function zombie.client:draw()
    ...
    and wmy > vy - img:getHeight() and wmy < vy then
        ...
        local name = string.format('Zombie [Level %i]', self.level)
