...
for _, sc in ipairs{'server', 'client'} do
    newMonster2[sc].newDefaults = function()
        local t = {
            id = lume.uuid(),
            x = 0, y = 0,
            xv = 0, yv = 0,
    ...
    newMonster2[sc].spawn = function(self)
    ...
    newMonster2[sc].destroy = function(self)
    ...
    newMonster2[sc].type = 'newMonster2'
    newMonster2[sc].static = false
    newMonster2[sc].enemy = true
end



function newMonster2.server:serialize()
    local t = {}
    for _, v in ipairs{
        'id', 'type',
        'x', 'y', 'xv', 'yv',
...
function newMonster2.server:update(dt)
...
function newMonster2.server:damage(d, clientId)
    self.hp = self.hp - d
...
function newMonster2.client:new(o)
    o = o or {}
...
function newMonster2.client:setState(state)
...
function newMonster2.client:lerpState(a, b, t)
    local state = {}
...
function newMonster2.client:update(dt)
...
function newMonster2.client:draw()
    ...
    and wmy > vy - img:getHeight() and wmy < vy then
        ...
        local name = string.format('NM2 [Level %i]', self.level)
