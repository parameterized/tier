...
for _, sc in ipairs{'server', 'client'} do
    smallRock[sc].newDefaults = function()
        local t = {
            id = lume.uuid(),
            x = 0, y = 0
        }
    ...
    smallRock[sc].destroy = function(self)
    ...
    smallRock[sc].type = 'smallRock'
end



function smallRock.client:draw()
    local img = gfx.environment.smallRock
