...
for _, sc in ipairs{'server', 'client'} do
    stingy[sc].newDefaults = function()
        local t = {
            id = lume.uuid(),
            x = 0, y = 0,
            xv = 0, yv = 0,
    ...
    stingy[sc].destroy = function(self)
    ...
    stingy[sc].type = 'stingy'
    stingy[sc].static = false
    stingy[sc].enemy = true
end



function stingy.server:serialize()
    local t = {}
    for _, v in ipairs{
        'id', 'type',
        'x', 'y', 'xv', 'yv',
...
function stingy.server:update(dt)
...
function stingy.server:damage(d, clientId)
    self.hp = self.hp - d
...
function stingy.client:setState(state)
...
function stingy.client:lerpState(a, b, t)
    local state = {}
...
function stingy.client:update(dt)
...
function stingy.client:draw()
    ...
    and wmy > vy - img:getHeight() and wmy < vy then
        ...
        local name = string.format('Stingy [Level %i]', self.level)
