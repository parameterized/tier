...
for _, sc in ipairs{'server', 'client'} do
    mudskipper[sc].newDefaults = function()
        local t = {
            id = lume.uuid(),
            x = 0, y = 0,
            xv = 0, yv = 0,
    ...
    mudskipper[sc].spawn = function(self)
    ...
    mudskipper[sc].destroy = function(self)
    ...
    mudskipper[sc].type = 'mudskipper'
    mudskipper[sc].static = false
    mudskipper[sc].enemy = true
end



function mudskipper.server:serialize()
    local t = {}
    for _, v in ipairs{
        'id', 'type',
        'x', 'y', 'xv', 'yv',
...
function mudskipper.server:update(dt)
...
function mudskipper.client:new(o)
    o = o or {}
...
function mudskipper.client:setState(state)
...
function mudskipper.client:lerpState(a, b, t)
    local state = {}
...
function mudskipper.client:update(dt)
...
function mudskipper.client:draw()
    ...
    local quad = anims.enemies.mudskipper.quads[frame]
    local _, _, w, h = quad:getViewport()
    love.graphics.push()
    local vx, vy = self.body:getPosition()
    love.graphics.translate(lume.round(vx), lume.round(vy))
    love.graphics.draw(anims.enemies.mudskipper.sheet, quad,
    ...
    and wmy > vy - h and wmy < vy then
        ...
        local name = string.format('Mudskipper [Level %i]', self.level)
