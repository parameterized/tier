...
for _, sc in ipairs{'server', 'client'} do
    sorcerer[sc].newDefaults = function()
        local t = {
            id = lume.uuid(),
            x = 0, y = 0,
            xv = 0, yv = 0,
    ...
    sorcerer[sc].spawn = function(self)
    ...
    sorcerer[sc].destroy = function(self)
    ...
    sorcerer[sc].type = 'sorcerer'
    sorcerer[sc].static = false
    sorcerer[sc].enemy = true
end



function sorcerer.server:serialize()
    local t = {}
    for _, v in ipairs{
        'id', 'type',
        'x', 'y', 'xv', 'yv',
...
function sorcerer.server:update(dt)
...
function sorcerer.server:damage(d, clientId)
    serverEnemyDamage(self, d, clientId)
end



function sorcerer.client:new(o)
    o = o or {}
...
function sorcerer.client:setState(state)
...
function sorcerer.client:lerpState(a, b, t)
    local state = {}
...
function sorcerer.client:update(dt)
...
function sorcerer.client:draw()
    ...
    and wmy > vy - img:getHeight() and wmy < vy then
        ...
        local name = string.format('Sorcerer [Level %i]', self.level)
